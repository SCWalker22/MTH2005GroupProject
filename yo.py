...
x = input("Height of cloud: ")


# Run model

def main(r0, N_val, W_val, z_top=int(x)):
    global N, W
...
# Parameter ranges (do whatever you want)

r0_vals = [0.5e-6, 1e-6, 2e-6, 5e-6]
N_vals = [50e6, 100e6, 200e6, 400e6]
W_vals = [0.2, 0.5, 1.0, 2.0]
...
print("\nResults for varying initial droplet radius:")

for i in range(len(r0_vals)):
    print(f"Initial droplet radius = {r0_vals[i]*1e6} micrometres")
    print(f"Cloud top droplet radius = {r_final_r0[i]} micrometres")
    print(f"Maximum supersaturation = {S_max_r0[i]}")
    print("")

print("\nResults for varying droplet number concentration:")

for i in range(len(N_vals)):
    print(f"Droplet number concentration = {N_vals[i]/1e6} cm^-3")
    print(f"Cloud top droplet radius = {r_final_N[i]} micrometres")
    print(f"Maximum supersaturation = {S_max_N[i]}")
    print("")

print("\nResults for varying vertical velocity:")

for i in range(len(W_vals)):
    print(f"Vertical velocity = {W_vals[i]} m/s")
    print(f"Cloud top droplet radius = {r_final_W[i]} micrometres")
    print(f"Maximum supersaturation = {S_max_W[i]}")
    print("")
...
plt.subplot(2, 3, 1)
plt.plot([r*1e6 for r in r0_vals], r_final_r0, 'o-')
plt.xlabel('r0 (um)')
plt.ylabel('r_final (um)')
plt.title('Final radius vs Initial Radius')

plt.subplot(2, 3, 4)
plt.plot([r*1e6 for r in r0_vals], S_max_r0, 'o-')
plt.xlabel('r0 (um)')
plt.ylabel('S_max')
plt.title('Max S vs Initial Radius')

plt.subplot(2, 3, 2)
plt.plot([N/1e6 for N in N_vals], r_final_N, 'o-')
plt.xlabel('N (cm^-3)')
plt.title('Final radius vs N')

plt.subplot(2, 3, 5)
plt.plot([N/1e6 for N in N_vals], S_max_N, 'o-')
plt.xlabel('N (cm^-3)')
plt.title('Max S vs N')

plt.subplot(2, 3, 3)
plt.plot(W_vals, r_final_W, 'o-')
plt.xlabel('W (m/s)')
plt.title('Final radius vs W')

plt.subplot(2, 3, 6)
plt.plot(W_vals, S_max_W, 'o-')
plt.xlabel('W (m/s)')
